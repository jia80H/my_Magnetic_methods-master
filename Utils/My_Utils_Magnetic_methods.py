...
def R_to_V(R, e):
    ...
    :return: 体积
    """
    V = np.round(((4)*(np.pi)*(R**3)*e) / 3, decimals=3)
    return V
...
def grid(zmax, map_lenght):
    ...
    参数:
    zmax (int): 网格的深度大小
    map_lenght (float): 网格的长度大小

    返回:
    X (numpy.ndarray): 网格中深度方向的坐标
    Y (numpy.ndarray): 网格中长度方向的坐标
    """

    X = np.linspace(-map_lenght, map_lenght, zmax).reshape(zmax, 1)
    Y = np.linspace(-map_lenght, map_lenght, zmax).reshape(1, zmax)
    # 通过使用NumPy中的linspace函数，在范围
    # [-map_lenght, map_lenght] 上等距地生成 zmax 个坐标点。
    # 将这些坐标点reshape成为形状为 (zmax, 1) 的二维数组X，
    # 代表地图中每个点的x坐标。
    # 将这些坐标点reshape成为形状为 (1, zmax)
    # 的二维数组Y，代表地图中每个点的y坐标。
    return X, Y
...
def latitudes_array(lat_min, lat_max, lat_frequency):
    ...
    Args:
        lat_min (float): 最小纬度
        lat_max (float): 最大纬度
        lat_frequency (float): 纬度频率

    Returns:
        tuple: 包含纬度数组、转换为弧度的纬度数组以及计算出的倾角数组
    """
    lat_array = np.arange(lat_min, lat_max, lat_frequency)
    lat_array_rad = lat_array * (np.pi/180)

    I = np.arctan(2*np.tan(lat_array_rad))  # 计算倾角

    return lat_array, lat_array_rad, I


def r_V_h_array(r_min, r_max, r_frequency, eAboughtLAndR,
                h_min, h_max, h_frequency, position, zmax, Br):

    r_array = np.arange(r_min, r_max, r_frequency)
    V_array = R_to_V(r_array, eAboughtLAndR)
    h_array = np.arange(h_min, h_max, h_frequency)

    n_examples = r_array.shape[0] * h_array.shape[0] * position.shape[0]

    Parameters_array = np.zeros((n_examples, 6))

    i_n = 0

    for i_r in r_array:
        for i_h in h_array:
            for i_p in range(position.shape[0]):
                Parameters_array[i_n, 0] = i_r
                Parameters_array[i_n, 1] = i_h
                Parameters_array[i_n, 2] = position[i_p, 0]
                Parameters_array[i_n, 3] = position[i_p, 1]
                Parameters_array[i_n, 4] = position[i_p, 2]
                Parameters_array[i_n, 5] = position[i_p, 3]
                i_n = i_n + 1

    return r_array, V_array, h_array, n_examples, Parameters_array
...
def X_array_raw(Br, m, I, X, Y, h, H_capteur_bas, H_capteur_haut, N_lat, examples, zmax, n_parameters, lat_array_rad, Parameters_array):
    ...
    Args:
        Br (numpy.ndarray): 磁通密度值的数组
        m (numpy.ndarray): 磁性异常值的数组
        I (numpy.ndarray): 仪器的俯仰角的数组
        X (numpy.ndarray): X坐标值的数组
        Y (numpy.ndarray): Y坐标值的数组
        h (numpy.ndarray): 深度值的数组
        H_capteur_bas (int): 传感器的最低高度
        H_capteur_haut (int): 传感器的最高高度
        N_lat (int): 纬度数
        examples (int): 例子数
        zmax (int): z轴最大值
        n_parameters (int): 参数数
        lat_array_rad (numpy.ndarray): 纬度值的数组
        Parameters_array (numpy.ndarray): 参数值的数组
    ...
    X_array_raw = np.zeros((N_lat, examples, zmax, zmax))

    # Array with volume and depth values
    Parameters_array_raw = np.zeros((N_lat, examples, n_parameters))
    ...
    for i_lat_rad in lat_array_rad:
        ...
        r = np.sqrt(np.add(np.add(X**2, Y**2),
                    (h**2).reshape(h.shape[0], 1, 1)))

        costeta = np.divide(
            ((X * np.cos(I[i])) - (np.multiply(h.reshape(h.shape[0], 1, 1), np.sin(I[i])))), r)

        Anomalie = np.outer(m.reshape(m.shape[0], 1, 1), np.divide(
            3*(costeta**2) - 1, r**3)).reshape((examples, zmax, zmax))
...
def Plot_X_array_raw(map_lenght, height, width, example_to_plot, lat_array,
                     X_data_array_raw, Parameters_array_raw, n_model_aug,
                     n_dipoles, rows, cols, zmax):
    ...
    for lat_i in range(n_lat):
        cp = axs[lat_i].contourf(
            X, Y, X_data_array_raw[lat_i, example_to_plot, :, :],
            levels=16, cmap='rainbow')
        axs[lat_i].set_xlabel('位置X (m)')
        axs[lat_i].set_ylabel('位置Y (m)')
        axs[lat_i].set_xticks(np.arange(-25, 25, step=2))
        axs[lat_i].set_yticks(np.arange(-25, 25, step=2))

        title = lat_array[lat_i]
        axs[lat_i].set_title(f'纬度 = {title}°')

        clb = fig.colorbar(cp, ax=axs[lat_i])
        clb.set_label('磁通密度 (nT)',
                      labelpad=15, y=0.5, rotation=270)

    plt.tight_layout()


def plot_contour_raw(map_lenght, height_2, width_2, example_to_plot,
                     X_data_array_raw, outer_x, outer_y,
                     box_anomaly, lat_array):
    ...
    if outer_x != 0:
        print(
            f"outer_X: {int(outer_x + (zmax/2))}, outer_Y: {int(outer_y + (zmax/2))}")
    ...
    for lat_i in range(n_lat):
        ...
        rect_pixel = Rectangle((x0_2, y0_2), bwidth_2,
                               bheight_2, edgecolor='r', facecolor="none")

        cp_1 = axs[(2*lat_i)].contourf(X, Y, X_data_array_raw[lat_i,
                                                              example_to_plot, :, :], levels=40, cmap='seismic')
        ...
        cp_2 = axs[(2*lat_i)+1].imshow(X_data_array_raw[lat_i,
                                                        example_to_plot, :, :])
...
def Plot_X_data(map_lenght, height_2, width_2, example_to_plot, X_data_array_raw, Position_f, box_anomaly, n_dipoles, inclination, n_lat_to_plot):
    ...
    for lat_i in range(n_lat_to_plot):
        ...
        for n_ii in range(n_dipoles):
            ...
            x0 = (bbox[lat_i, example_to_plot, (5*n_ii)+1] -
                  (zmax/2)) * pixel_to_real
            y0 = (bbox[lat_i, example_to_plot, (5*n_ii)+2] -
                  (zmax/2)) * pixel_to_real
            bwidth = bbox[lat_i, example_to_plot, (5*n_ii)+3] * pixel_to_real
            bheight = bbox[lat_i, example_to_plot, (5*n_ii)+4] * pixel_to_real

            rect_real.append(Rectangle((x0, y0), bwidth,
            ...
            rect_pixel.append(Rectangle((x0_2, y0_2), bwidth_2,
                              bheight_2, edgecolor='r', facecolor="none"))

        cp_1 = axs[(2*lat_i)].contourf(X, Y, X_data_array_raw[lat_i,
                                                              example_to_plot, :, :], levels=18, cmap='seismic')
        axs[(2*lat_i)].set_xlabel('Position X (m)')
        axs[(2*lat_i)].set_ylabel('Position Y (m)')
        axs[(2*lat_i)].set_xticks(np.arange(-25, 25, step=2))
        axs[(2*lat_i)].set_yticks(np.arange(-25, 25, step=2))
...
def Plot_X_data_final(map_lenght, height_2, width_2, example_to_plot, X_data_array_raw, Position_f, box_anomaly, n_dipoles, lat_array_f, n_lat):
    ...
    for n_ii in range(n_dipoles):
        ...
        rect_real.append(Rectangle((x0, y0), bwidth, bheight,
                         edgecolor='r', facecolor="none"))
        ...
        print(
            f"Dipole_{n_ii}: x0: {position[example_to_plot,n_ii,0]}, y0: {position[example_to_plot,n_ii,1]}, w: {int(bwidth_2)}, h:{int(bheight_2)}")

    cp_1 = axs[(0)].contourf(
        X, Y, X_data_array_raw[example_to_plot, :, :], levels=18, cmap='seismic')
    ...
    cp_2 = axs[1].imshow(
        X_data_array_raw[example_to_plot, :, :], origin="lower")
...
def padding_array(pad, padder, X_data_array_raw):
    ...
    X_data_pad = np.zeros((X_data_array_raw.shape[0], X_data_array_raw.shape[1],
                          X_data_array_raw.shape[2]+(2*pad), X_data_array_raw.shape[2]+(2*pad)))

    for i in range(0, X_data_array_raw.shape[0], 1):

        for n_pad in range(0, X_data_array_raw.shape[1], 1):

            X_data_pad[i, n_pad, :, :] = np.pad(
                X_data_array_raw[i, n_pad, :, :], pad, pad_with)
...
def X_array(N_models, N_dipoles, N_lat, zmax, examples, X_data_pad, Parameters_array_raw, conversion):
    ...
    Parameters_array = np.zeros(
        (N_models, N_dipoles, N_lat, examples, Parameters_array_raw.shape[2]))
    Position_array = np.zeros(
        (N_models, N_dipoles, N_lat, examples, N_dipoles*Parameters_array_raw.shape[2]))
    ...
    for n_models_i in range(0, N_models, 1):
        ...
        for n_dipoles_i in range(0, N_dipoles, 1):
            ...
            Position_array_to_add = np.zeros(
                (N_lat, examples, Parameters_array_raw.shape[2]))
            ...
            for i_lat in range(0, N_lat, 1):
                ...
                for n_examples in range(0, examples, 1):
                    ...
                    X_data_x = np.roll(
                        X_data_pad[i_lat, n_examples, :, :], position_x)
                    X_data_y[i_lat, n_examples, :, :] = np.roll(
                        X_data_x, position_y, axis=0)

                    Lat_counting[n_models_i, n_dipoles_i,
                                 i_lat, n_examples, 0] = i_lat
                    N_dipoles_counting[n_models_i, n_dipoles_i,
                                       i_lat, n_examples, 0] = n_dipoles_i
            ...
            if n_dipoles_i == 0:
                ...
                X_data_array[n_models_i, n_dipoles_i, :, :,
                             :, :] = X_data_y[:, :, 64:192,  64:192]

                Position_array[n_models_i, n_dipoles_i:, :, :, (n_dipoles_i)*2:(
                    n_dipoles_i+1)*2] = (Position_array_to_add[:, :, :]) * conversion

                Parameters_array[n_models_i, n_dipoles_i,
                                 :, :, :] = Parameters_array_raw[:, :, :]

            else:

                X_data_array[n_models_i, n_dipoles_i, :, :, :, :] = X_data_y[:, :,
                                                                             64:192,  64:192] + X_data_array[n_models_i, n_dipoles_i-1, :, :, :, :]
...
def Y_array(N_models, N_dipoles, N_lat, zmax, n_examples, Parameters_array, Position_array, e):

    Y_data_array = np.zeros(
        (N_models, N_dipoles, N_lat, n_examples, zmax, zmax))
        ...
        for i_dipoles in range(0, N_dipoles, 1):
            ...
            for i_lat in range(0, N_lat, 1):
                ...
                for i_examples in range(0, n_examples, 1):
                    # 缺少e
                    Y_r = V_to_R(
                        Parameters_array[n_models_i, i_dipoles, i_lat, i_examples, 0], e)

                    for i_circle in range(0, i_dipoles+1, 1):

                        Y_pos_x = (
                            Position_array[n_models_i, i_dipoles, i_lat, i_examples, (i_circle*2)]) / (50/zmax)
                        Y_pos_y = (
                            Position_array[n_models_i, i_dipoles, i_lat, i_examples, (i_circle*2)+1]) / (50/zmax)

                        rr, cc = draw.circle(
                            (zmax/2)+Y_pos_y, (zmax/2)+Y_pos_x, Y_r)

                        Y_data_array[n_models_i, i_dipoles,
                                     i_lat, i_examples, rr, cc, 0] = i_lat
...
def gaussian_noise_models(mean, var, sigma, n_models_with_noise, zmax, X_data_array, Y_data_array):
    ...
    for i_noise_models in range(0, n_models_with_noise, 1):
        ...
        X_data_array_with_noise[i_noise_models, :, :, :, :,
                                :] = X_data_array[i_noise_models, :, :, :, :, :] + gaussian

    Y_data_array_with_noise = Y_data_array[:
                                           n_models_with_noise, :, :, :, :, :, :]
...
def noise_models_visualization(zmax, map_lenght, height, width, example_to_plot, lat_array, X_data_array_with_noise_final, Lat_counting_noise_final, N_dipoles_counting_noise_final):
    ...
    if example_to_plot == 'random':
        ...
        example_to_plot = int(np.random.randint(
            0, (X_data_noise.shape[0]-6), 1))
    ...
    for i_model_n in range(example_to_plot, example_to_plot+6, 1):
        ...
        plt.subplot(3, 3, i + 1)
        plt.contourf(
            X, Y, X_data_noise[i_model_n, :, :], levels=1, cmap='rainbow')
        plt.axis('on')
        plt.xlabel('Position X (m)')
        plt.ylabel('Position Y (m)')

        plt.title('Lat: ' + str(int(Lat_counting_noise_final[i_model_n])) +
                  'N_dipoles: ' + N_dipoles_counting_noise_final[i_model_n])

        clb = plt.colorbar()
        clb.set_label(parameter_to_plot + '(m^3)',
                      labelpad=15, y=0.5, rotation=270)
